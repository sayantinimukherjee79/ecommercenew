...
function BestDeal() {
    ...
    // Pagination
    const STORAGE_KEY = "bestDealCurrentPage";
    ...
    useEffect(() => {
        const fetchBestDealProducts = async () => {
            try {
                const res = await axios.get("http://localhost:5000/products/sale");
    ...
    const handleClothingChange = (type) =>
        setSelectedClothing(prev => prev.includes(type) ? prev.filter(t => t !== type) : [...prev, type]);
